...
impl BitArray {
    ...
    pub fn get_next_set(&self, from: usize) -> usize {
        ...
        let mut bits_offset = self.size / 32;
        ...
        let result = bits_offset * 32 + self.number_of_trailing_zeros(current_bits);
        if result > self.size {
            return self.size;
        } else {
            return result;
    ...
    fn number_of_trailing_zeros(&self, mut n: i32) -> usize {
        let mut i: usize = 0;
        if n == 0 {
            return 32;
        }
    ...
    pub fn set_bulk(&mut self, i: usize, new_bits: i32) {
        self.bits[i / 32] = new_bits;
    }

    pub fn set_range(&mut self, start: usize, mut end: usize) {
        if end < start || end > self.size {
            panic!();   // TODO IllegalArgumentException
        }
    ...
    pub fn is_range(&self, start: usize, mut end: usize, value: bool) -> bool {
        if end < start || end > self.size {
            panic!();   // TODO IllegalArgumentException
        }
    ...
    pub fn to_bytes(&self, mut bit_offset: usize, mut array: Vec<u8>, offset: usize, num_bytes: usize) {
